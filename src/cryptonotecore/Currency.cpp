...
namespace CryptoNote
{
        ...
        uint32_t height) const
    {
        ...
        /* Masternode reward */
        if (height >= CryptoNote::parameters::MASTERNODE_ENABLE_HEIGHT) {
            baseReward += CryptoNote::parameters::MASTERNODE_BLOCK_REWARD;
        }

        /* Staking reward */
        if (height >= CryptoNote::parameters::STAKING_ENABLE_HEIGHT) {
            baseReward += CryptoNote::parameters::STAKING_BLOCK_REWARD;
        }

        /* Mesh reward */
        if (height >= CryptoNote::parameters::MESH_ENABLE_HEIGHT) {
            baseReward += CryptoNote::parameters::MESH_BLOCK_REWARD;
        }
        ...
        size_t maxOuts /* = 1*/) const
    {
        ...
        if(height >= CryptoNote::parameters::MASTERNODE_ENABLE_HEIGHT) {
            blockReward -= CryptoNote::parameters::MASTERNODE_BLOCK_REWARD;
        }

        /* Reduce staking reward from block reward */
        if(height >= CryptoNote::parameters::STAKING_ENABLE_HEIGHT) {
            blockReward -= CryptoNote::parameters::STAKING_BLOCK_REWARD;
        }

        /* Reduce mesh reward from block reward */
        if(height >= CryptoNote::parameters::MESH_ENABLE_HEIGHT) {
            blockReward -= CryptoNote::parameters::MESH_BLOCK_REWARD;
        }
        ...
        if(height >= CryptoNote::parameters::MASTERNODE_ENABLE_HEIGHT) {
            decompose_amount_into_digits(
                CryptoNote::parameters::MASTERNODE_BLOCK_REWARD,
                defaultDustThreshold(height),
                [&outAmounts](uint64_t a_chunk) { outAmounts.push_back(a_chunk); },
                [&outAmounts](uint64_t a_dust) { outAmounts.push_back(a_dust); });
        }
        ...
        if(height >= CryptoNote::parameters::STAKING_ENABLE_HEIGHT) {
            decompose_amount_into_digits(
                CryptoNote::parameters::STAKING_BLOCK_REWARD,
                defaultDustThreshold(height),
                [&outAmounts](uint64_t a_chunk) { outAmounts.push_back(a_chunk); },
                [&outAmounts](uint64_t a_dust) { outAmounts.push_back(a_dust); });
        }
        ...
        if(height >= CryptoNote::parameters::MESH_ENABLE_HEIGHT) {
            decompose_amount_into_digits(
                CryptoNote::parameters::MESH_BLOCK_REWARD,
                defaultDustThreshold(height),
                [&outAmounts](uint64_t a_chunk) { outAmounts.push_back(a_chunk); },
                [&outAmounts](uint64_t a_dust) { outAmounts.push_back(a_dust); });
        }
