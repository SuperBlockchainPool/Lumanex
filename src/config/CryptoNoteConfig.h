...
namespace CryptoNote
{
  ...
  {
    const uint64_t DIFFICULTY_TARGET                                       = 10;                              /* Every block takes 10 seconds */
    const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX                 = 0x2935;                          /* Wallet addresses prefix: XL */
    ...
    const uint64_t MONEY_SUPPLY                                            = UINT64_C(75'000'000'00000000);   /* 21'000'000 LNX */
    const uint64_t BLOCK_REWARD                                            = UINT64_C(1'00000000);            /* 1.00000000 LNX */
    const uint64_t GENESIS_BLOCK_REWARD                                    = UINT64_C(750'000'00000000);      /* 750'000.00000000 LNX (1% Premine) */
    const uint64_t MASTERNODE_BLOCK_REWARD                                 = UINT64_C(3'00000000);            /* Masternode Reward: 3.00000000 LNX */
    const uint64_t STAKING_BLOCK_REWARD                                    = UINT64_C(2'00000000);            /* Staking Reward: 2.00000000 LNX */
    const uint64_t MESH_BLOCK_REWARD                                       = UINT64_C(1'00000000);            /* Mesh Reward: 1.00000000 LNX */

    const uint32_t MASTERNODE_ENABLE_HEIGHT                                = 777600;                          /* Masternodes will be enabled at height 777,600 */
    const uint32_t STAKING_ENABLE_HEIGHT                                   = 777600;                          /* Staking will be enabled at height 777,600 */
    const uint32_t MESH_ENABLE_HEIGHT                                      = 5000000;                         /* Proof of Mesh (PoM) enable height. TBA */
    ...
    const char GENESIS_COINBASE_TX_HEX[] = 
      "010a01ff000180c2d72f029b2e4c0281c0b02e7c53291a94d1d0cbff8883f8024f5142ee494ffbbd08807121018f3b4dc26c17097046c65a0b906b058800dfce61e48afaf84915971e98bb8645";
    ...
    /* This is the unix timestamp of the first "mined" block (technically block 2, not the genesis block)
       You can get this value by doing "print_block 2" in Lumanexd. It is used to know what timestamp
       to import from when the block height cannot be found in the node or the node is offline. */
    const uint64_t GENESIS_BLOCK_TIMESTAMP                                 = 1694048400;                    /* 03:00 AM CEST, September 7 2023 */
    ...
    /* Fee to charge per byte of transaction. Will be applied in chunks, see
       ...
       - 5 LNX vs 5.12 LNX. You can read this as.. the fee per chunk
    ...
    const uint64_t ACCEPTABLE_FEE                                          = UINT64_C(100);     /* with this fee, tx is always accepted whatever size - 0.00000100 LNX */
    const uint64_t ACCEPTABLE_FEE_V1                                       = UINT64_C(100);     /* with this fee, tx is always accepted whatever size - 0.00000100 LNX */
    ...
    /* 15,000,000 LNX -> Max supply / mixin+1 outputs                 */
    /* This is enforced on the daemon side. An output > 15,000,000 causes an invalid block.   */
    const uint64_t MAX_OUTPUT_SIZE_NODE                                    = 26'000'000'00000000;

    /* 500,000 LNX */
    /* This is enforced on the client side. An output > 500,000 will not */
  ...
  const char    CRYPTONOTE_NAME[]                                          = "Lumanex";
  ...
  const int      P2P_DEFAULT_PORT                                          = 45000;           // P2P Port
  const int      RPC_DEFAULT_PORT                                          = 45001;           // RPC Port
  const int      SERVICE_DEFAULT_PORT                                      = 45002;           // Service Port
  ...
  const char LATEST_VERSION_URL[]                                          = "https://github.com/Lumanex-Project/Lumanex/releases";
  const std::string LICENSE_URL                                            = "https://github.com/Lumanex-Project/Lumanex/blob/master/LICENSE";

  const static boost::uuids::uuid CRYPTONOTE_NETWORK = {
    {0x6b, 0x93, 0x1d, 0xf8, 0x72, 0x5a, 0x4e, 0x20, 0xc7, 0x84, 0xda, 0x61, 0x39, 0xae, 0x7f, 0xb0}
  };

  const char *const SEED_NODES[] = {
    "185.250.36.146:45000",
    "185.250.36.146:45100"
  };
}
