...
namespace WalletConfig
{
    ...
    const std::string_view addressPrefix = "XL";

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = "LNX";
    ...
    const std::string daemonName = "Lumanexd";

    /* The name to call this wallet */
    const std::string walletName = "Lumanex-Wallet";

    /* The name of service/walletd, the programmatic rpc interface to a
       wallet */
    const std::string walletdName = "Lumanex-Service";

    /* The full name of your crypto */
    const std::string coinName = std::string(CryptoNote::CRYPTONOTE_NAME);

    /* Where can your users contact you for support? E.g. discord */
    const std::string contactLink = "https://discord.gg/MzCpu8uYec";
    ...
    /* The default fee value to use with transactions (in ATOMIC units!) - 500 LNX */
    const uint64_t defaultFee = CryptoNote::parameters::MINIMUM_FEE;

    /* The minimum fee value to allow with transactions (in ATOMIC units!)  - 500 LNX */
